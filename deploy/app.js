...
async function uploadDropbox(path, content) {
  ...
  const r = await fetch('https://content.dropboxapi.com/2/files/upload', {
    method: 'POST',
    headers: { Authorization: `Bearer ${process.env.DROPBOX_TOKEN}`, 'Dropbox-API-Arg': JSON.stringify({ path, mode: 'overwrite', autorename: false, mute: false, strict_conflict: false }), 'Content-Type': 'application/octet-stream' },
...
app.get('/health', (_, res) => res.json({ ok: true, openaiConfigured: Boolean(process.env.OPENAI_API_KEY), dropboxConfigured: Boolean(process.env.DROPBOX_TOKEN), dropboxRoot: DROPBOX_ROOT, maxFileChars: MAX_FILE_CHARS, maxQueueItems: MAX_QUEUE_ITEMS, deepResearch: true, time: new Date().toISOString() }));
...
app.post('/job', (req, res) => { const payload = { ...(req.body?.payload || {}) }; if (typeof payload.content === 'string') { const clamped = clampText(payload.content); payload.content = clamped.text; payload.contentLength = clamped.originalLength; payload.truncated = Boolean(payload.truncated || clamped.truncated); } if (typeof payload.objective === 'string') { const clamped = clampText(payload.objective); payload.objective = clamped.text; payload.objectiveLength = clamped.originalLength; payload.truncated = Boolean(payload.truncated || clamped.truncated); } const job = addJob(req.body?.type || 'agent_plan', payload, req.body?.priority || 0); res.json(safeJob(job)); });
