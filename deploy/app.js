...
app.get('/health', (_, res) => res.json({ ok: true, openaiConfigured: Boolean(process.env.OPENAI_API_KEY), dropboxConfigured: Boolean(process.env.DROPBOX_TOKEN), dropboxRoot: DROPBOX_ROOT, time: new Date().toISOString() }));
...
app.post('/dropbox/save', async (_, res) => {
  try {
    ...
    const uploaded = [];
    uploaded.push(await uploadDropbox(`${DROPBOX_ROOT}/queue-${stamp}.json`, JSON.stringify({ exportedAt: new Date().toISOString(), jobs }, null, 2)));
